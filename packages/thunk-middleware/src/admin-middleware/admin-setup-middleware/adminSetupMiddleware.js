...
export const fetchAdminList = (path, queryParams, searchData) => async dispatch => {
...
export const deleteAdmin = (path, deleteData) => async dispatch => {
    ...
    try {
        ...
        dispatch(AdminSetupActions.adminDeleteSuccess(response.data ? response.data : "Admin deleted successfully."));
        return response;
    } catch (e) {
...
export const editAdmin = (path, editData, formData) => async dispatch => {
    ...
    try {
        const response = await Axios.putWithMultiPart(path, 'request', editData, formData);
        dispatch(AdminSetupActions.adminEditSuccess(response.data));
        return response;
    } catch (e) {
        dispatch(AdminSetupActions.adminEditError(e.errorMessage ? e.errorMessage : 'Sorry error occurred while editing admin.'));
        throw e;
    }
};
...
export const previewAdmin = (path, id) => async dispatch => {
    ...
    try {
        ...
        dispatch(AdminSetupActions.adminPreviewError(e.errorMessage ? e.errorMessage : 'Sorry,Internal Server Error!'));
        throw e;
    }
};
...
export const fetchAdminMetaInfo = (path) => async dispatch => {
    ...
    try {
        ...
        dispatch(AdminSetupActions.adminMetaInfoFetchError(e.errorMessage ? e.errorMessage : 'Sorry,Internal Server Error!'));
        throw e;
    }
};
