...
class PreviewClientProfileModalContent extends React.PureComponent {
    ...
    handleParentMenuClick = childMenusOfParent => {
        ...
        this.setState({
            activeKey:
                menuToGetRolesOf.parentId !== null
                    ? menuToGetRolesOf.parentId
                    : menuToGetRolesOf.id,
            activeKeyChild:
                menuToGetRolesOf.parentId !== null ? menuToGetRolesOf.id : '',
            childMenusOfSelectedParent: Array.isArray(childMenusOfParent)
                ? childMenusOfParent
                : [],
    ...
    getTabsAndRolesForSelectedMenu(childMenu) {
        ...
        const selectedRoles = this.props.profileData.selectedMenus.filter(
            menu => Number(menu.userMenuId) === Number(childMenu.id)
        )
        ...
        roles.map(role => {
            if (
                childMenu.roles.includes(role.id) &&
                selectedRoles.find(
                    selectedRole =>
        ...
        let tabsWithRoles = Array.from(tabs).map(tab => {
            return {
                ...
                roles: rolesForSelectedMenu.filter(
                    role => role.parent_role_id === tab.id
                )
            }
        })
            ...
            <>
                ...
                <Container-fluid>
                    <Row className="view-assigned-menu">
                        {profileData.selectedUserMenusForModal &&
                        profileData.selectedUserMenusForModal.length !== 0 ? (
                            <>
                                <Col sm={12} md={6} lg={3} className="menu-list-wrapper">
                                    <div className="assign-menu">
                                        <div className="am-header">
                                            <span className="am-title">Menus</span>
                                        </div>
                                        <CScrollbar
                                            id="menus"
                                            autoHide={true}
                                        ...
                                        >
                                            <Accordion activeKey={this.state.activeKey}>
                                                {profileData.selectedUserMenusForModal.length !== 0 &&
                                                profileData.selectedUserMenusForModal.map(
                                                    userMenu => (
                                                        <Card key={userMenu.id}>
                                                            <Accordion.Toggle
                                                                eventKey={userMenu.id}
                                                                key={userMenu.id}
                                                                as={Card.Header}
                                                                className={
                                                                    this.state.activeKey === userMenu.id
                                                                        ? 'activeParent'
                                                                        : ''
                                                                }
                                                                onClick={() =>
                                                                    this.handleParentMenuClick(
                                                                        userMenu.childMenus.length
                                                                            ? userMenu.childMenus
                                                                            : userMenu
                                                                    )
                                                                }
                                ...
                                <Col sm={12} md={6} lg={3} className="menu-list-wrapper">
                                    <div className="assign-menu">
                                        <div className="am-header">
                                            <span className="am-title">Sub Menus</span>
                                        </div>
                                        <CScrollbar
                                            id="menus"
                                            autoHide={true}
                                            style={{height: 313}}
                                        >
                                            <Accordion activeKey={this.state.activeKeyChild}>
                                                {this.state.childMenusOfSelectedParent.length !== 0 &&
                                                this.state.childMenusOfSelectedParent.map(
                                                    childMenu => (
                                                        <Card key={childMenu.id}>
                                                            <Accordion.Toggle
                                                                eventKey={childMenu.id}
                                                                key={childMenu.id}
                                                                as={Card.Header}
                                                                className={
                                                                    this.state.activeKeyChild === childMenu.id
                                                                        ? 'activeChild'
                                                                        : ''
                                                                }
                                                                onClick={() =>
                                        ...
                                        >
                                            <div className="assign-body">
                                                {this.state.tabsWithRolesForSelectedMenu.map(
                                                    tabWithRoles => (
                                                        <div className="assign-item" key={tabWithRoles.id}>
                                                            <div
                                                                key={tabWithRoles.id}
                            ...
                            <Col sm={12} md={6} lg={3} className="menu-list-wrapper">
                                <div className="assign-menu">
                                    <div className="filter-message">
                                        ...
                                        <div className="message text-center">No Menus Selected</div>
                                    </div>
                                </div>
                            </Col>
                        )}
