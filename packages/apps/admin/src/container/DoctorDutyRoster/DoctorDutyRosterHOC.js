    ...
    class DoctorDutyRoster extends PureComponent {
        ...
        checkManageFormValidity = () => {
            const {
                ...
                weekDaysDutyRosterUpdateRequestDTOS, isCloneAndAdd, hospital, specialization, doctor
            } = this.state.updateDoctorDutyRosterData;

            let formValid = isCloneAndAdd ? rosterGapDuration && hospital && specialization && doctor
        ...
        searchDoctorDutyRoster = async page => {
            const {fromDate, toDate, hospital, specialization, doctor, status} = this.state.searchParameters;
            if (isFirstDateGreaterThanSecondDate(fromDate, toDate) &&
                getNoOfDaysBetweenGivenDatesInclusive(fromDate, toDate) !== 1
            ) {
                this.setState({
