import {unitSetupConstants} from '@frontend-appointment/action-module'

const {
    DATA_FETCH_ERROR,
    SAVE_UNITS,
    CREATE_UNIT_PENDING,
    CREATE_UNIT_SUCCESS,
    CREATE_UNIT_ERROR,
    UNIT_LIST_PENDING,
    UNIT_LIST_SUCCESS,
    UNIT_LIST_ERROR,
    UNIT_EDIT_PENDING,
    UNIT_EDIT_SUCCESS,
    UNIT_EDIT_ERROR,
    UNIT_DELETE_PENDING,
    UNIT_DELETE_SUCCESS,
    UNIT_DELETE_ERROR,
    UNIT_PREVIEW_ERROR,
    UNIT_PREVIEW_SUCCESS,
    UNIT_PREVIEW_PENDING,
    CLEAR_UNIT_CREATE_SUCCESS_MESSAGE,
    CLEAR_UNIT_CREATE_ERROR_MESSAGE,
    CLEAR_UNIT_LIST_FETCH_ERROR_MESSAGE,
    CLEAR_UNIT_EDIT_SUCCESS_MESSAGE,
    CLEAR_UNIT_EDIT_ERROR_MESSAGE,
    CLEAR_UNIT_PREVIEW_ERROR_MESSAGE,
    CLEAR_UNIT_DELETE_ERROR_MESSAGE,
    CLEAR_UNIT_DELETE_SUCCESS_MESSAGE,
    FETCH_UNIT_BY_HOSPITAL_ID_SUCCESS,
    FETCH_UNIT_BY_HOSPITAL_ID_ERROR,
    FETCH_ACTIVE_UNIT_FOR_DROPDOWN_ERROR,
    FETCH_ACTIVE_UNIT_FOR_DROPDOWN_PENDING,
    FETCH_ACTIVE_UNIT_FOR_DROPDOWN_SUCCESS
} = unitSetupConstants;

const initialState = {
    departments: [],
    isCreateDepartmentLoading: false,
    errorMessage: '',//departmentCreate error message
    successMessage: '',
    isSearchLoading: false,
    departmentList: [],
    searchErrorMessage: '',
    deleteErrorMessage: '',
    deleteSuccessMessage: '',
    isDeleteLoading: false,
    isDepartmentEditLoading: false,
    departmentErrorMessage: '',//department edit error message
    departmentSuccessMessage: '',
    departmentPreviewData: {},
    departmentPreviewErrorMessage: '',
    departmentPreviewOpen: false,
    departmentsByHospital: [],
    errorMessageForDropdown: '',
    isFetchActiveDepartmentsForDropdownPending: false,
    activeDepartmentsForDropdown: [],
    dropdownErrorMessage: ''
};

export const UnitSetupReducer = (state = {...initialState}, action) => {
    switch (action.type) {
        case SAVE_UNITS:
            return {
                ...state,
                departments: [...action.payload.data]
            };
        case DATA_FETCH_ERROR:
            return {
                ...state,
                errorMessage: action.payload.errorMessage
            };
        case CREATE_UNIT_PENDING:
            return {
                ...state,
                isCreateDepartmentLoading: true,
            };
        case CREATE_UNIT_SUCCESS:
            return {
                ...state,
                isCreateDepartmentLoading: false,
                successMessage: action.payload.successMessage
            };
        case CLEAR_UNIT_CREATE_SUCCESS_MESSAGE:
            return {
                ...state,
                successMessage: ''
            };
        case CREATE_UNIT_ERROR:
            return {
                ...state,
                isCreateDepartmentLoading: false,
                errorMessage: action.payload.errorMessage
            };
        case CLEAR_UNIT_CREATE_ERROR_MESSAGE:
            return {
                ...state,
                isCreateDepartmentLoading: false,
                errorMessage: ''
            };
        case FETCH_UNIT_BY_HOSPITAL_ID_SUCCESS:
            return {
                ...state,
                departmentsByHospital: [...action.payload.data]
            };
        case FETCH_UNIT_BY_HOSPITAL_ID_ERROR:
            return {
                ...state,
                departmentsByHospital: [],
                errorMessageForDropdown: action.payload.errorMessage
            };
        default:
            return state
    }
};

export const UnitListReducer = (state = {...initialState}, action) => {
    switch (action.type) {
        case UNIT_LIST_PENDING:
            return {
                ...state,
                departmentList: [...state.departmentList],
                isSearchLoading: true,
                searchErrorMessage: ''
            };
        case UNIT_LIST_SUCCESS:
            return {
                ...state,
                departmentList: [...action.payload.data],
                isSearchLoading: false,
                searchErrorMessage: ''
            };
        case UNIT_LIST_ERROR:
            return {
                ...state,
                departmentList: [...state.departmentList],
                isSearchLoading: false,
                searchErrorMessage: action.payload.data
            };
        case CLEAR_UNIT_LIST_FETCH_ERROR_MESSAGE:
            return {
                ...state,
                searchErrorMessage: ''
            };
        default:
            return {...state}
    }
};

export const UnitDeleteReducer = (state = {...initialState}, action) => {
    switch (action.type) {
        case UNIT_DELETE_PENDING:
            return {
                ...state,
                deleteSuccessMessage: '',
                deleteErrorMessage: '',
                isDeleteLoading: true
            };
        case UNIT_DELETE_SUCCESS:
            return {
                ...state,
                deleteSuccessMessage: action.payload.data,
                deleteErrorMessage: '',
                isDeleteLoading: false
            };
        case UNIT_DELETE_ERROR:
            return {
                ...state,
                deleteSuccessMessage: '',
                deleteErrorMessage: action.payload.data,
                isDeleteLoading: false
            };
        case CLEAR_UNIT_DELETE_ERROR_MESSAGE:
            return {
                ...state,
                deleteErrorMessage: ''
            };
        case CLEAR_UNIT_DELETE_SUCCESS_MESSAGE:
            return {
                ...state,
                deleteSuccessMessage: ''
            };
        default:
            return {
                ...state
            }
    }
};

export const UnitEditReducer = (state = {...initialState}, action) => {
    switch (action.type) {
        case UNIT_EDIT_PENDING:
            return {
                ...state,
                isDepartmentEditLoading: true,
                departmentErrorMessage: '',
                departmentSuccessMessage: ''
            };
        case UNIT_EDIT_SUCCESS:
            return {
                ...state,
                isDepartmentEditLoading: false,
                departmentErrorMessage: '',
                departmentSuccessMessage: "Unit Edited successfully."
            };
        case UNIT_EDIT_ERROR:
            return {
                ...state,
                isDepartmentEditLoading: false,
                departmentErrorMessage: action.payload.data,
                departmentSuccessMessage: ''
            };
        case CLEAR_UNIT_EDIT_SUCCESS_MESSAGE:
            return {
                ...state,
                departmentSuccessMessage: ''
            };
        case CLEAR_UNIT_EDIT_ERROR_MESSAGE:
            return {
                ...state,
                departmentErrorMessage: ''
            };
        default:
            return {...state}
    }
};

export const UnitPreviewReducer = (state = {...initialState}, action) => {
    switch (action.type) {
        case UNIT_PREVIEW_PENDING :
            return {
                ...state,
                departmentPreviewOpen: false,
                departmentPreviewData: {}
            };
        case UNIT_PREVIEW_SUCCESS:
            return {
                ...state,
                departmentPreviewErrorMessage: '',
                departmentPreviewOpen: true,
                departmentPreviewData: {...action.payload.data}
            };
        case UNIT_PREVIEW_ERROR:
            return {
                ...state,
                departmentPreviewErrorMessage: action.payload.errorMessage,
                departmentPreviewOpen: false,
                departmentPreviewData: {}
            };
        case CLEAR_UNIT_PREVIEW_ERROR_MESSAGE:
            return {
                ...state,
                departmentPreviewErrorMessage: ''
            };
        default:
            return {...state}
    }
};

// todo to be used after refactoring department setup
export const UnitDropdownReducer = (state = {...initialState}, action) => {
    switch (action.type) {
        case FETCH_ACTIVE_UNIT_FOR_DROPDOWN_PENDING:
            return {
                ...state,
                isFetchActiveDepartmentsForDropdownPending: true,
                activeDepartmentsForDropdown: [],
                dropdownErrorMessage: ''
            };
        case FETCH_ACTIVE_UNIT_FOR_DROPDOWN_SUCCESS:
            return {
                ...state,
                isFetchActiveDepartmentsForDropdownPending: false,
                activeDepartmentsForDropdown: [...action.payload.data],
                dropdownErrorMessage: ''
            };
        case FETCH_ACTIVE_UNIT_FOR_DROPDOWN_ERROR:
            return {
                ...state,
                isFetchActiveDepartmentsForDropdownPending: false,
                activeDepartmentsForDropdown: [],
                dropdownErrorMessage: action.payload.errorMessage
            };
        default:
            return state
    }
};
