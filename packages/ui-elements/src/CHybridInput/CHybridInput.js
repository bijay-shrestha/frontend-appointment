...
class CHybridInput extends React.PureComponent {
    ...
    classAdditionWhenValueIsChanged = value => {
        ...
        let className = a ? a.className : null;
        if (!(value === 0 || value)) {
            if (className) {
                if (className.includes('hasValue')) {
                    const replaceString = className.replace(' hasValue', '')
        ...
        if (this.props.isInvalid && !className.includes('errorInput')) {
            ReactDOM.findDOMNode(
                this.refs['fieldWrapper' + this.props.id]
