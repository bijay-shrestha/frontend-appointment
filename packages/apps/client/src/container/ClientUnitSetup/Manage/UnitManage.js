...
import {logoutUser, UnitSetupMiddleware} from "@frontend-appointment/thunk-middleware";
...
class UnitManage extends PureComponent {
    ...
    onEditHandler = async id => {
        try {
            await this.previewApiCall(id);
            const {name, departmentCode, status} = this.props.UnitPreviewReducer.departmentPreviewData;
            this.setState({
                showEditModal: true,
                departmentUpdateData: {
                    ...this.state.departmentUpdateData,
