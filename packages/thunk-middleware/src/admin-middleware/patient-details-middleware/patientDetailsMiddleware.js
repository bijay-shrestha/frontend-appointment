...
export const fetchPatientMetaDropdownWithoutHospitalId = path => async dispatch => {
  dispatch(
    PatientDetailsActions.patientActiveDropdownFetchingStartWithoutHospitalId()
  )
  try {
    const response = await Axios.get(path)
    dispatch(
...
export const fetchPatientDetailByAppointmentId = (
  path,
  appointmentNumber
) => async dispatch => {
  ...
  try {
    ...
    dispatch(
      PatientDetailsActions.fetchPatientDetailByAppointmentNumberSuccess(
        response.data
      )
    )
    return response.data
    ...
    dispatch(
      PatientDetailsActions.fetchPatientDetailByAppointmentNumberError(
...
export const fetchEsewaDetails = (path, hospitalId) => async dispatch => {
  dispatch(PatientDetailsActions.fetchEsewaIdForDropdownPending())
  try {
    let response = null
    if (hospitalId)
      response = await Axios.getWithPathVariables(path, hospitalId)
    else response = await Axios.get(path)
    dispatch(
      PatientDetailsActions.fetchEsewaIdForDropdownSuccess(response.data)
    )
    return response.data
  } catch (e) {
    dispatch(
      PatientDetailsActions.fetchEsewaIdForDropdownError(
        e.errorMessage ? e.errorMessage : 'Sorry internal server error!'
      )
    )
  }
